...
- **M1 IMPLEMENTADO e com roteiro de teste completo PASSANDO em 2026-07-04** (commits `0c1e7a7` + fixes `111d5dc`/`a9dc6e2`; console limpo verificado em log exportado pelo usuário). Aprendizado incorporado: tokens não-vinculados têm ator sintético — flags de opt-out vivem lá; `setPromptEnabled` cobre os dois lados. **Aguardando aceite formal do M1.** Teste local: módulo em `Data\modules\boss-forge` é junction para o repo (mundo mostra versão 0.0.0).
