...
- **M2 ACEITO em 2026-07-04** ("podemos riscar o M2 do roadmap"). Entrega: dialog GM segurando `midi-qol.preSavesComplete` (relay via socketlib quando o workflow roda no cliente do player) + cobertura do caminho nativo (anúncio de queima desacoplado da visibilidade do save via `forceSuccess`; auto-burn opcional em cards com CD). Aprendizados: a mesa rola saves de monstro fora do workflow (privados) e o botão nativo do dnd5e no card cobre a queima manual com fricção baixa — o usuário prefere UI no card a dialogs; settings agora são declarativas (`SETTINGS_SCHEMA`); `"socket": true` no manifest (exige reload do mundo).
