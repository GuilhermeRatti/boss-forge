...
- **M2 ACEITO em 2026-07-04** ("podemos riscar o M2 do roadmap"). Entrega: dialog GM segurando `midi-qol.preSavesComplete` (relay via socketlib quando o workflow roda no cliente do player) + caminho nativo coberto (anúncio de queima + FX desacoplados da visibilidade do save, via `forceSuccess` do botão do card). **Auto-burn e timeout do dialog foram implementados e removidos a pedido do usuário** — filosofia da mesa: automação mínima, GM no controle, sem timers. Outros aprendizados: a mesa rola saves de monstro fora do workflow (privados) e o usuário prefere UI no card a dialogs; settings declarativas (`SETTINGS_SCHEMA`); `"socket": true` no manifest (exige reload do mundo).
...
- **M2 — Resistência lendária** ✅ aceito (2026-07-04): save falho → dialog GM (workflow do Midi, relay socketlib) ou botão nativo no card → sucesso forçado + decremento de `legres`, com anúncio configurável no chat.
