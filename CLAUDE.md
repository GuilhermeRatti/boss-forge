...
- **M4 IMPLEMENTADO em 2026-07-04**, núcleo aprovado no 1º teste (transições/FX/mensagens/one-way); 2ª iteração adicionou os gatilhos dos cenários do usuário: fases por CURA (`direction: "up"`), mortes vinculadas (`linkDeaths` — banshees→ritual cessa→boss enfraquece) e diferença de HP entre dois bosses (`linkGap` — enrage de consortes); aguardando teste final. Design em `docs/design/m4-fases.md` (§7); pesquisa em `docs/research/2026-07-04-m4-fases-fontes-instaladas.md`. Fatos-chave: `hp.pct` nativo; gatilho `updateActor`; detecção por estado one-way; ações por fase = filtro nos prompts M1/M3; auras = ActiveEffects por nome.
