...
- **M5 em 2026-07-04: parte 1 (motor + presets `impact`/`telegraph`/`beam`/`rain`/`aura`) TESTADA E APROVADA; parte 2 (Forja de FX — catálogo `ApplicationV2`) implementada, aguardando teste.** Identidade visual própria "forja" (carvão + brasa `--bf-ember`, cantos chanfrados via clip-path, glow de calor, temas claro/escuro) — inspirada no BLFX (norte indicado pelo usuário, frames do vídeo analisados) sem copiar. Formulários gerados dos metadados do registro; preview no token; aplicar em item/legres/covil; botão GM na barra de tokens; `api.fx.openCatalog()`. Design em `docs/design/m5-biblioteca-presets.md` (backlog: telegraph por turnos); pesquisa em `docs/research/2026-07-04-m5-sequencer-fontes-instaladas.md`.
