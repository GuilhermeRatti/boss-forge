...
- **M4 IMPLEMENTADO em 2026-07-04**, aguardando teste/feedback. Design em `docs/design/m4-fases.md`; pesquisa em `docs/research/2026-07-04-m4-fases-fontes-instaladas.md`. Fatos-chave: `hp.pct` é derivado nativo (sobre `effectiveMax`); gatilho = `updateActor` (cobre Midi/cards/edição manual — `dnd5e.applyDamage` não cobre manual); detecção por estado (fase calculada vs `phaseState.index`, one-way); troca de ações = filtro por flag de fase nos prompts M1/M3; auras = toggle de ActiveEffects por nome.
...
- **M4 — Fases** (implementado em 2026-07-04, aguardando teste/feedback): thresholds de % de HP → anúncio público + FX + toggle de efeitos; ações trocam por fase via flag de item filtrada nos prompts.
