...
- **Settings de módulo** (world): `legresPrompt` (boolean, default true — liga/desliga o recurso) e `legresChatVisibility` (choices `public`/`gm`, default `public` — visibilidade da mensagem de queima). Registro de settings passa a ser **declarativo** (tabela → loop, i18n derivada da chave) para organizar o crescimento previsto de configurações sem retrabalho. *(`legresAutoBurn` e `legresTimeout` foram implementados e depois removidos — ver §10.)*
- **Sem timeout no dialog** (ajuste final da revisão): a queima é decisão do GM e o dialog espera por ela; ESC/fechar = "não queimar". Sem GM ativo conectado, o relay do socketlib falha com erro capturado e o save permanece falho.
...
└─ para cada candidato (sequencial):
   ...
   │    [Queimar] / [Não queimar] / [Não perguntar mais p/ este boss] (ESC/fechar = não queimar)
...
- GM desconectado / sem GM ativo → o `executeAsGM` falha, o erro é capturado e o save permanece falho; log debug.
...
1. **Anúncio desacoplado da visibilidade do save**: ouvinte de `updateChatMessage` detecta `flags.dnd5e.roll.forceSuccess = true` (botão nativo) e publica a mensagem de queima conforme `legresChatVisibility` — e toca o FX do ator — mesmo com o save privado.
2. **Auto-burn e timeout: implementados e REMOVIDOS** a pedido do usuário no mesmo dia ("essas funcionalidades não me parecem boas") — filosofia da mesa: automação mínima, GM no controle, sem timers artificiais.
3. **Escopo final (validado em teste): o dialog de workflow foi REMOVIDO por inteiro.** Nas palavras do usuário: "não devemos nem tentar [dialog]; vamos continuar com o botão nativo porque é bem limpo, simples e prático, serve perfeitamente. Só vamos tocar na mensagem de aviso e na possibilidade de adicionar animações". Com isso caem também o relay socketlib (`"socket": true` removido do manifest — volta se outro milestone comprovar necessidade), o opt-out por ator e o setting de prompt (vira `legresAnnounce`). **A mensagem não revela usos restantes** — informação de GM, não de jogador.

**Papel final do Boss Forge no legres**: anúncio no chat (visibilidade configurável) + FX opcional por ator, disparados pela queima nativa. **M2 ACEITO em 2026-07-04** ("podemos riscar o M2 do roadmap"); anúncio e FX validados em teste pelo usuário no mesmo dia.
