...
A Action então: compila os packs, carimba versão e URLs no `module.json` (derivados da tag e do repositório, então continuam corretos em forks/renomeações), gera `module.zip` e publica uma GitHub Release com `module.zip` + `module.json` anexados.
